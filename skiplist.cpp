...
skip_list *create_sl()
{
    ...
    sl->head = h;
    int i;
	 // 将header的next数组清空遍 历头节点的 next 数组，将每个指针初始化为 NULL，表示当前没有任何后继节点。
    for(i=0; i<MAX_L; ++i)
    {
        h->next[i] = NULL;
    }
	srand(time(0)); // 使用当前时间设置随机数种子，以便在后续操作中生成随机层数时得到不同的结果。
    return sl;
}
...
// 查找
// 查找
valueType* search(skip_list* sl, keyType key, int* query_count)
{
    Node* q, * p = sl->head;
    q = NULL;
    *query_count = 0;  // 初始化查询次数
    ...
    for (; i >= 0; --i)
    {
        ...
        {
            p = q;
            (*query_count)++;  // 每次移动都增加查询次数
        }
        if (q && key == q->key)
        {
            (*query_count)++;  // 找到时也增加查询次数
            return &(q->value);
        }
    }
